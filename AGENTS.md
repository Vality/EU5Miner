...
The public `eu5miner.inspection` entity-browsing seam also keeps repeated entity queries in a process-local cache keyed by install, system, and mod-root context. Contributor-owned mutating workflows that can change inspection results should invalidate affected systems through `invalidate_system_entity_cache(...)` or otherwise refresh inspection cache state; the higher-level `apply_mod_update(...)` path already does this automatically when it still has install-backed context, while lower-level direct write helpers stay inspection-agnostic and must not grow hidden cache side effects.
