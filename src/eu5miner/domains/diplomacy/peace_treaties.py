"""Domain adapter for peace treaty definitions."""
...
class PeaceTreatyColumn:
    """One column definition inside a peace treaty select trigger."""
...
@dataclass(frozen=True)
class PeaceTreatySelectTrigger:
    """One select_trigger block inside a peace treaty."""
    ...
    columns: tuple[PeaceTreatyColumn, ...]
...
@dataclass(frozen=True)
class PeaceTreatyDefinition:
    """One peace treaty definition."""
    ...
    cost: SemanticObject | None
    base_antagonism: str | SemanticObject | None
    antagonism_type: str | None
    ai_desire: SemanticObject | None
    blocks_full_annexation: bool | None
    collate_targets: bool | None
    are_targets_exclusive: bool | None
    ai_force_add: bool | None
    category: str | None
    custom_tags: tuple[str, ...]
    show_tags_in_ui: bool | None
    select_triggers: tuple[PeaceTreatySelectTrigger, ...]
    entry: SemanticEntry
...
@dataclass(frozen=True)
class PeaceTreatyDocument:
    """Parsed peace treaty file."""

    definitions: tuple[PeaceTreatyDefinition, ...]
    semantic_document: SemanticDocument

    def names(self) -> tuple[str, ...]:
        return names_from_named(self.definitions)

    def get_definition(self, name: str) -> PeaceTreatyDefinition | None:
        return get_by_name(self.definitions, name)


def parse_peace_treaty_document(text: str) -> PeaceTreatyDocument:
    semantic_document = parse_semantic_document(text)
    definitions: list[PeaceTreatyDefinition] = []
    ...
    for entry in semantic_document.entries:
        ...
        definitions.append(
            PeaceTreatyDefinition(
                ...
                cost=body.get_object("cost"),
                base_antagonism=_parse_scalar_or_object(body.first_entry("base_antagonism")),
                antagonism_type=body.get_scalar("antagonism_type"),
                ai_desire=body.get_object("ai_desire"),
                blocks_full_annexation=parse_bool_or_none(
    ...
    return PeaceTreatyDocument(
        definitions=tuple(definitions),
        semantic_document=semantic_document,
    )


def _parse_select_triggers(body: SemanticObject) -> tuple[PeaceTreatySelectTrigger, ...]:
    select_triggers: list[PeaceTreatySelectTrigger] = []
...
def _parse_columns(trigger_body: SemanticObject) -> tuple[PeaceTreatyColumn, ...]:
    columns: list[PeaceTreatyColumn] = []
