    ...
    for entry in semantic_document.entries:
        ...
        definitions.append(
            GenericActionDefinition(
                ...
                show_message=_parse_bool_or_none(body.get_scalar("show_message")),
                show_message_to_target=_parse_bool_or_none(
                    body.get_scalar("show_message_to_target")
                ),
                should_execute_price=_parse_bool_or_none(
                    body.get_scalar("should_execute_price")
                ),
                show_in_gui_list=_parse_bool_or_none(body.get_scalar("show_in_gui_list")),
                ...
                force_click_and_confirm_or_hold=_parse_bool_or_none(
    ...
    for entry in body.find_entries("select_trigger"):
        ...
        select_triggers.append(
            GenericActionSelectTrigger(
                ...
                cache_targets=_parse_bool_or_none(trigger_body.get_scalar("cache_targets")),
                cache_interaction_source_list=_parse_bool_or_none(
                    trigger_body.get_scalar("cache_interaction_source_list")
                ),
                cache_order=_parse_bool_or_none(trigger_body.get_scalar("cache_order")),
                name=trigger_body.get_scalar("name"),
                allow_null=_parse_bool_or_object(trigger_body.first_entry("allow_null")),
                allow_self=_parse_bool_or_none(trigger_body.get_scalar("allow_self")),
                move_to_next_section_on_click=_parse_bool_or_none(
                ...
                show_why_not_visible=_parse_bool_or_none(
                    trigger_body.get_scalar("show_why_not_visible")
                ),
                show_why_not_enabled=_parse_bool_or_none(
    ...
    for entry in trigger_body.find_entries("column"):
        ...
        columns.append(
            GenericActionColumn(
                ...
                show_icon_in_cells=_parse_bool_or_none(
...
def _parse_bool_or_object(entry: SemanticEntry | None) -> bool | SemanticObject | None:
    ...
    if isinstance(entry.value, SemanticObject):
        return entry.value
    if isinstance(entry.value, SemanticScalar):
        return _parse_bool_or_none(entry.value.text)
    return None
