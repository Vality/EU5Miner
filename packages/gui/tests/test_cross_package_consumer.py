...
These tests exercise the ``eu5miner_gui`` desktop controller using the real
...
def test_controller_reports_supported_systems_for_synthetic_install(tmp_path: Path) -> None:
    ...
    assert not missing, (
        ...
        "system-rendering code in src/eu5miner_gui/desktop/."
    )
...
def _eu5miner_target_module(stripped_line: str) -> str | None:
    ...
    ``from eu5miner_gui ...`` is NOT confused with ``from eu5miner ...``.
    """
...
def test_gui_does_not_reach_into_private_library_modules() -> None:
    ...
    import eu5miner_gui  # noqa: F401  (just importing for the side effect)
    gui_modules = [
        name
        for name in sys.modules
        if name.startswith("eu5miner_gui.")
    ]
    ...
    for module_name in gui_modules:
        ...
        for line in text.splitlines():
            ...
            # avoids confusing ``from eu5miner_gui ...`` with ``from eu5miner ...``.
