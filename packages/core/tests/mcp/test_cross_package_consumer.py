...
These tests exercise the ``eu5miner.mcp`` server using the real ``eu5miner``
...
from eu5miner.mcp.server import MCPServer, build_server
...
from eu5miner.mcp.tools.entities import (
...
from eu5miner.mcp.tools.systems import (
...
_ALLOWED_EU5MINER_IMPORT_PREFIXES = (
    ...
    "eu5miner.vfs",
    # After consolidation, ``eu5miner.mcp`` lives in the same wheel. Intra-package
    # imports between the MCP server, transport, models, serializers, and tools
    # are no longer "reaching into the library" — they're internal MCP plumbing.
    "eu5miner.mcp",
)
...
def _eu5miner_target_module(stripped_line: str) -> str | None:
    ...
    ``from eu5miner.mcp ...`` is NOT confused with ``from eu5miner ...``.
    """
...
def test_mcp_does_not_reach_into_private_library_modules() -> None:
    ...
    import eu5miner.mcp  # noqa: F401  (just importing for the side effect)
    mcp_modules = [
        name
        for name in sys.modules
        if name.startswith("eu5miner.mcp.")
    ]
...
def test_mcp_and_library_agree_on_supported_system_names() -> None:
    ...
    enum.
    """
    from eu5miner.mcp.tools import describe_system_tools
...
def test_mcp_and_library_agree_on_entity_system_names() -> None:
    ...
    browsing surface.
    """
    from eu5miner.mcp.tools import describe_entity_tools
