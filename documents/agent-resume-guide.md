...
- attribute-column adapter for top-level object-type groups with typed column definitions, normalized width and row-layout flags, preserved sort blocks, and shared support for generic-action selection UIs, first validated against `common/attribute_columns/00_defaults.txt`, `19_market.txt`, and `31_goods.txt`
...
- shipped diplomacy content now has a higher-level graph/report helper over peace treaties, casus belli, subject types, country interactions, and character interactions; the helper tracks resolved and missing references across `casus_belli:`, `subject_type:`, and `country_interaction:` links, including embedded references inside longer script-value strings
- shipped `common/unit_types/*.txt` files are flat top-level unit definitions with a stable core of recruitment metadata, tag lists, optional recruitment trigger blocks, repeated `mercenaries_per_location` blocks, terrain-specific `combat` and `impact` objects, and a shared scalar modifier vocabulary that overlaps heavily with unit categories
- shipped `common/unit_abilities/*.txt` files are flat top-level ability definitions with a compact stable schema built around trigger/effect blocks (`hidden`, `allow`, `finished_when`, `start_effect`, `finish_effect`, `on_entering_location`), simple toggles for army/navy/map/combat behavior, and optional animation/UI fields such as `confirm` and entity states
- shipped `common/unit_categories/*.txt` files are flat top-level category definitions with stable identity booleans (`is_army`, `assault`, `bombard`, `auxiliary`, `is_garrison`, `transport`), demand/build fields, optional terrain `combat` blocks, and the same scalar modifier vocabulary used by unit types
- shipped `common/government_types/00_default.txt` is a compact flat family of top-level government identities with repeated `heir_selection` scalars, a few stable booleans such as `use_regnal_number` and `generate_consorts`, one government-power identity field, and a preserved modifier block
- shipped `common/government_reforms/*.txt` is the richer adjacent family: flat top-level reform definitions with optional `government` scoping, booleans like `major`, `unique`, and `block_for_rebel`, timer fields (`years`, `months`, `weeks`, `days`), optional name lists, and one or more modifier/effect/trigger blocks such as `country_modifier`, `location_modifier`, `potential`, `allow`, `locked`, `on_activate`, and `on_deactivate`
- shipped `common/laws/*.txt` is the real source for both laws and policies: each top-level law definition carries law metadata plus nested policy objects, while `common/policies/policies.info` is only a placeholder noting that policies are created indirectly from laws
- shipped `common/estates/00_default.txt` is a compact top-level estate identity file with stable scalar power/tax fields, a few boolean identity flags, and preserved modifier-style blocks such as `satisfaction`, `high_power`, `low_power`, `power`, and `opinion`
- shipped `common/estate_privileges/*.txt` is the richer adjacent family: flat top-level privilege definitions keyed by privilege name, linked back to one `estate`, with optional `potential`, `allow`, `can_revoke`, timed activation fields, repeated modifier blocks, and optional activation/deactivation effects
- shipped `common/parliament_types/*.txt` is a compact top-level family keyed by parliament type name with stable `type`, optional `potential`/`allow`/`locked`, and one preserved modifier block covering both country and international-organization variants
- shipped `common/parliament_agendas/*.txt` is a richer top-level agenda family with repeated `estate` or `special_status` selectors, optional `potential`/`allow`, `on_accept`, `on_bribe`, `can_bribe`, `ai_will_do`, and scalar-or-script `chance` and `importance`
- shipped `common/parliament_issues/*.txt` is a parallel top-level issue family with repeated `estate` or `special_status`, optional `modifier_when_in_debate`, `allow`, `potential`, `selectable_for`, debate outcome effects, scalar-or-script `chance`, and optional `wants_this_parliament_issue_bias`
- shipped `common/institution/*.txt` is a flat top-level institution family with stable identity fields such as `age` and optional `location`, plus script-heavy `can_spawn`, `promote_chance`, and multiple `spread_*` fields that are best normalized as scalar-or-object values without over-modeling each formula
- shipped `common/holy_site_types/*.txt` is a compact top-level family keyed by type name with stable `country_modifier`, `location_modifier`, and `religion_modifier` blocks
- shipped `common/holy_sites/*.txt` is a flat top-level holy-site family with stable `location`, `type`, `importance`, `religions`, and optional `god`/`avatar`; the `type` link is strong enough to justify a small helper catalog/report for type resolution and religion-based lookup
- shipped `common/societal_values/*.txt` is a flat top-level axis family with stable `left_modifier`, `right_modifier`, and occasional scalar weighting fields such as `opinion_importance_multiplier`
- shipped `common/religious_aspects/*.txt` is a flat top-level family with repeated `religion` selectors, optional `visible` and `enabled` trigger blocks, one `modifier` block, and an `opinions` object keyed by other aspect ids
- shipped `common/religious_factions/*.txt` is a compact top-level family with `visible`, `enabled`, and an `actions` list of generic-action ids
- shipped `common/religious_focuses/*.txt` is a top-level research-style family with optional `potential` and `allow`, required `monthly_progress`, optional completion/progress modifier blocks, and optional completion effects
- shipped `common/religious_schools/*.txt` is a flat top-level family keyed by school id with stable `color`, `enabled_for_country`, `enabled_for_character`, and `modifier`
- shipped `common/religious_figures/*.txt` is a minimal top-level family keyed by figure id with one `enabled_for_religion` trigger block
- the higher-level religion helper now links religions to explicit factions, focuses, and school lists, reverse-linked aspects and holy sites, and trigger-matched schools and figures where the shipped files express those links through religion or religion-group checks

## Next Planned Work

The diplomacy and warfare layer now includes typed adapters for `common/casus_belli/*.txt`, `common/wargoals/*.txt`, `common/peace_treaties/*.txt`, `common/subject_types/*.txt`, `common/subject_military_stances/*.txt`, `common/country_interactions/*.txt`, `common/character_interactions/*.txt`, `common/unit_types/*.txt`, `common/unit_abilities/*.txt`, and `common/unit_categories/*.txt`. The government and society phase now includes typed adapters for `common/government_types/*.txt`, `common/government_reforms/*.txt`, `common/laws/*.txt`, `common/estates/*.txt`, `common/estate_privileges/*.txt`, `common/parliament_types/*.txt`, `common/parliament_agendas/*.txt`, `common/parliament_issues/*.txt`, `common/institution/*.txt`, `common/holy_site_types/*.txt`, `common/holy_sites/*.txt`, `common/societal_values/*.txt`, `common/religious_aspects/*.txt`, `common/religious_factions/*.txt`, `common/religious_focuses/*.txt`, `common/religious_schools/*.txt`, and `common/religious_figures/*.txt`, plus higher-level government, holy-site, and religion helper layers. The roadmap audit now indicates that the major game-system family phases through interface/presentation are covered; the next recommended step is the library integration pass plus a broader optional validation sweep.
