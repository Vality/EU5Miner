...
The checked-in docs now also reflect the coordinated `0.6.0` preview surface after that shipped step-2 breadth. The current focus is post-release operational follow-through rather than another helper-surface expansion.
...
The checked-in repo now reflects both the narrow tool-contract baseline and the shipped step-2 grouped-helper breadth through diplomacy and religion. With the `0.6.0` preview now published, the immediate next phase is continued validation, build, test, and contract-maintenance work over that surface.
...
- keep the registry, docs, and validation gate aligned with the actual shipped `0.6.0` tool surface while post-release stabilization work proceeds
